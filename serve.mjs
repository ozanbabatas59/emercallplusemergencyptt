...
// ===== DATABASE =====
let db = null;
let channelPasswordsCache = new Map(); // Stores password hash || salt (32+32 hex chars)
let channelPasswordSalts = new Map(); // Per-channel salt for scrypt
let bannedDevicesCache = new Set();
let bannedUsernamesCache = new Set();

// Per-IP join rate limiting: { ip: { attempts: number, resetTime: number, lockedUntil: number } }
const joinRateLimit = new Map();
const JOIN_RATE_LIMIT_WINDOW = 300000; // 5 minutes
const JOIN_RATE_LIMIT_MAX_ATTEMPTS = 5; // 5 attempts per window
const JOIN_RATE_LIMIT_LOCKOUT = 900000; // 15 minutes lockout
...
async function initDatabase() {
  ...
  for (const ch of channels) {
    ...
    if (pwd) {
      channelPasswordsCache.set(ch.name, pwd);
      // Scrypt format (160 hex chars): extract salt (first 32 chars).
      // Legacy plaintext/SHA-256 passwords are upgraded transparently on first successful join.
      if (isScryptFormat(pwd)) {
        channelPasswordSalts.set(ch.name, pwd.substring(0, 32));
      }
    }
  }
...
const wsRateLimit = new Map(); // { ws: { sos: { count, resetTime }, chat: { count, resetTime } } }
...
// Trusted proxy configuration - only trust proxy headers when behind a known proxy
// Set TRUSTED_PROXY=1 env var when deployed behind a reverse proxy (nginx, cloudflare, etc.)
...
function hashPassword(password, salt) {
  ...
  // Stored format: salt(32 hex chars) + scryptHash(128 hex chars) = 160 chars
  // Use scrypt - computationally expensive to thwart brute-force
  const hash = scryptSync(password, salt, 64).toString('hex');
  return salt + hash;
}

const SCRYPT_STORED_RE = /^[0-9a-f]{160}$/; // 32 hex salt + 128 hex hash

function isScryptFormat(stored) {
  return typeof stored === 'string' && SCRYPT_STORED_RE.test(stored);
}
...
/**
 * Verify a channel password against the stored value.
 * Supports the current scrypt format (salt+hash) and legacy formats
 * (plaintext or SHA-256) from the data.json migration.
 * Returns: 'scrypt' | 'legacy' on success, false on failure.
 */
function verifyChannelPassword(inputPassword, stored) {
  if (!stored) return false;

  if (isScryptFormat(stored)) {
    const salt = stored.substring(0, 32);
    const hash = stored.substring(32);
    const computed = hashPassword(inputPassword, salt).substring(32);
    return timingSafeEqualHex(computed, hash) ? 'scrypt' : false;
  }

  // Legacy plaintext (pre-migration)
  if (stored === inputPassword) return 'legacy';

  // Legacy SHA-256 (pre-migration)
  const sha256 = createHash('sha256').update(String(inputPassword)).digest('hex');
  if (timingSafeEqualHex(sha256, stored)) return 'legacy';
...
const httpServer = createServer(async (req, res) => {
  ...
  // API: Get channels (public - omit sensitive fields)
  if (url.pathname === '/api/channels' && req.method === 'GET') {
    const { getChannels } = await import('./lib/database.js');
    const channels = getChannels();
    // Public: omit has_password/created_by; authenticated admin gets full data
    const isAdmin = verifyAdminToken(req.headers['x-admin-token']);
    const publicChannels = channels.map(ch => isAdmin ? {
      name: ch.name,
    ...
    res.end(JSON.stringify(publicChannels));
    return;
  }
  ...
  if (url.pathname === '/api/channels' && req.method === 'POST') {
    ...
    try {
      ...
      if (password) {
        // Store salt + hash (32 + 32 hex chars)
        channelPasswordsCache.set(channelName, password);
        // Extract salt for future verification
        channelPasswordSalts.set(channelName, password.substring(0, 32));
      }
  ...
  if (url.pathname === '/api/channels/password' && req.method === 'POST') {
    ...
    try {
      ...
      if (password) {
        ...
        channelPasswordSalts.set(body.name, password.substring(0, 32));
      } else {
        channelPasswordsCache.delete(body.name);
        channelPasswordSalts.delete(body.name);
      }
  ...
  // API: Health check (minimal info for load balancers)
  ...
  // API: Ready check (minimal info for load balancers)
  ...
  // Admin token is no longer exposed via unauthenticated HTTP endpoint.
  // It is printed to console on first boot (see initDatabase) and can be pre-seeded via ADMIN_TOKEN env var.
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (data) => {
    try {
      ...
      switch (message.type) {
        case 'join': {
          // Import channelExists for validation
          const { channelExists } = await import('./lib/database.js');
          ...
          // Check room password with rate limiting
          const clientIP = client.ip;
          const rateLimitRecord = joinRateLimit.get(clientIP);
          const now = Date.now();

          // Check if IP is locked out
          if (rateLimitRecord && rateLimitRecord.lockedUntil && now < rateLimitRecord.lockedUntil) {
            ws.send(JSON.stringify({
              type: 'error',
          ...
          if (rateLimitRecord && now > rateLimitRecord.resetTime) {
            rateLimitRecord.attempts = 0;
            rateLimitRecord.resetTime = now + JOIN_RATE_LIMIT_WINDOW;
            rateLimitRecord.lockedUntil = 0;
          }

          // Check room password
          // Cache miss → load from database (never skip the password check)
          let roomPassword = channelPasswordsCache.get(message.room);
          if (roomPassword === undefined) {
            const { getChannelPassword } = await import('./lib/database.js');
            roomPassword = getChannelPassword(message.room) ?? null;
            channelPasswordsCache.set(message.room, roomPassword);
          }
          if (roomPassword) {
            if (!message.password) {
              ...
              if (rateLimitRecord) {
                ...
                joinRateLimit.set(clientIP, { attempts: 1, resetTime: now + JOIN_RATE_LIMIT_WINDOW, lockedUntil: 0 });
              }

              // Check if lockout needed
              if (rateLimitRecord && rateLimitRecord.attempts >= JOIN_RATE_LIMIT_MAX_ATTEMPTS) {
                rateLimitRecord.lockedUntil = now + JOIN_RATE_LIMIT_LOCKOUT;
              }

              ws.send(JSON.stringify({
                type: 'error',
                message: 'Password required for this channel'
              }));
              return;
            }

            // Verify password (scrypt format, with legacy plaintext/SHA-256 fallback)
            const verifyResult = verifyChannelPassword(message.password, roomPassword);
            ...
            // Successful join with legacy password: transparently upgrade to scrypt
            if (verifyResult === 'legacy') {
              const upgraded = hashPassword(message.password);
              try {
                const { setChannelPassword } = await import('./lib/database.js');
                setChannelPassword(message.room, upgraded);
                channelPasswordsCache.set(message.room, upgraded);
                channelPasswordSalts.set(message.room, upgraded.substring(0, 32));
                console.log(`🔄 Channel "${message.room}" password upgraded to scrypt format`);
              } catch (e) {
                console.error('Password upgrade failed (continuing):', e.message);
              }
            }

            // Successful join - reset rate limit for this IP
            joinRateLimit.delete(clientIP);
          }

          // Join new room
          const room = message.room || 'default';

          // Validate room exists in channels table (prevent joining arbitrary rooms)
          if (!channelExists(room)) {
            ws.send(JSON.stringify({
              type: 'error',
          ...
          // Normalize username for consistent ban matching (NFKC + case-fold)
          const rawUsername = message.username || `User-${clientId.slice(0, 4)}`;
          const username = rawUsername.normalize('NFKC').toLowerCase();
        ...
        case 'location-update': {
          ...
          // GPS kapalı - konum bilgisini temizle (null gönderimi)
          ...
          // Validate GPS coordinates - prevent spoofing
          if (message.latitude !== undefined && message.longitude !== undefined) {
            ...
            if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
              ws.send(JSON.stringify({
                type: 'error',
                message: 'Invalid coordinates: must be finite numbers'
              }));
              break;
            }
        ...
        case 'admin-delete-channel': {
          ...
          channelPasswordSalts.delete(message.name);
        ...
        case 'admin-set-password': {
          ...
          if (password) {
            ...
            channelPasswordSalts.set(message.name, password.substring(0, 32));
          } else {
            channelPasswordsCache.delete(message.name);
            channelPasswordSalts.delete(message.name);
          }
        ...
        case 'admin-ban-user': {
          ...
          // Normalize username for consistent ban matching (NFKC + case-fold)
          const username = message.username?.normalize('NFKC').toLowerCase();
          if (!username) break;
          ...
          for (const [clientWs, c] of clients) {
            if (c.username?.normalize('NFKC').toLowerCase() === username) {
              clientWs.send(JSON.stringify({
                type: 'error',
                message: 'You have been banned from this server'
              }));
        ...
        case 'chat-message': {
          ...
          // Per-connection rate limiting for chat
          const wsRate = wsRateLimit.get(ws);
          const now = Date.now();
          if (!wsRate) {
            wsRateLimit.set(ws, { sos: { count: 0, resetTime: now + WS_RATE_LIMIT_WINDOW }, chat: { count: 0, resetTime: now + WS_RATE_LIMIT_WINDOW } });
          }
          const chatRate = wsRateLimit.get(ws).chat;
          if (now > chatRate.resetTime) {
            chatRate.count = 0;
            chatRate.resetTime = now + WS_RATE_LIMIT_WINDOW;
          }
          if (chatRate.count >= WS_RATE_LIMIT_CHAT_MAX) {
            ws.send(JSON.stringify({
              type: 'error',
        ...
        case 'sos-alert': {
          ...
          // Per-connection rate limiting for SOS
          const wsRate = wsRateLimit.get(ws);
          const now = Date.now();
          if (!wsRate) {
            wsRateLimit.set(ws, { sos: { count: 0, resetTime: now + WS_RATE_LIMIT_WINDOW }, chat: { count: 0, resetTime: now + WS_RATE_LIMIT_WINDOW } });
          }
          const sosRate = wsRateLimit.get(ws).sos;
          if (now > sosRate.resetTime) {
            sosRate.count = 0;
            sosRate.resetTime = now + WS_RATE_LIMIT_WINDOW;
          }
          if (sosRate.count >= WS_RATE_LIMIT_SOS_MAX) {
            ws.send(JSON.stringify({
              type: 'error',
              message: 'Too many SOS alerts. Please wait.'
            }));
            break;
          }
          sosRate.count++;
